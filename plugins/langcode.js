...
✅ *Use these codes for translation and other language functions!*`;
    ...
    // Image URL: Fallback URL in case the first one fails
    const imageUrl = "https://files.catbox.moe/jf2g4q.jpeg";  // Custom image URL
    ...
    try {
      ...
      console.log("Error with image URL:", e);
      return reply("❌ Error fetching image. Please try again later.");
    }

    // Sending the message with language list and image
    await bot.sendMessage(from, {
      image: { url: imageUrl },  // Sending your custom image
      caption: languageList  // Sending the language list
    }, { quoted: message });

  } catch (error) {
    // Error handling
    console.log("An error occurred while processing the langcode command:", error);  // Detailed error logging
